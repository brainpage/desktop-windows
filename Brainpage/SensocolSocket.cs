...
namespace Brainpage
{
    class SensocolSocket
    {
        ...
        {
            Connect();
            FormState.GetInstance().UpdateConnectionStatus(Strings.connecting, false);
        }

        private void websocket_Closed(object sender, EventArgs e)
        {
            Utils.Log("Closed:" + e.ToString());
            Reconnect();
                ...
                switch (state)
                {
                    case CONNECTED:
                        ...
                        else if (response["action"].ToString() == "event")
                        {
                            if (response["event"].ToString() == "activity-update")
                            {
                                ...
                                FormState.GetInstance().UpdateActivity(Int16.Parse(data["percent"].ToString()));
                            }
                    ...
                    case DISCONNECTED:
                        if (response["action"].ToString() == "connack" && response["status"].ToString() == "ok")
                        {
                            ...
                            FormState.GetInstance().UpdateConnectionStatus(Strings.connected, true);
        ...
        private void Connect()
        {
            ...
            Utils.Log("uuid:" + AppData.GetInstance().SensorUUID);
            Utils.Log("token:" + AppData.GetInstance().AuthToken);
